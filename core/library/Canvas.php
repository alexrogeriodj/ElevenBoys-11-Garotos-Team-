...
class Canvas {
  ...
  private function resize_with_mycrop($center=true) {
    $fill_width = $new_width = $this->new_width;
    $fill_height = $new_height = $this->new_height;
    if($this->width/$this->height >= $new_width/$new_height) $fill_width = null; else $fill_height = null;
    $this->resize($fill_width, $fill_height, 'fill');
    if($center) $this->set_crop_coordinates(-($this->new_width - $new_width) / 2, -($this->new_height - $new_height) / 2);
    $this->resize($new_width, $new_height, 'crop');
    return $this;
  }
  
  private function resize_with_mycrop_zero() {
    return $this->resize_with_mycrop(false);
  }
